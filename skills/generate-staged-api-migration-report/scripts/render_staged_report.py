        ...
        for table_name, columns in grouped_fields.items():
            ...
            for column in target_fields:
                ...
                table_rows.setdefault(table_name, []).append(
                    {
                        "表字段": column,
                        "字段名": field_label(full_field, TARGET_FIELD_ALIASES),
...
def interface_view(data: dict[str, Any], interface_name: str, contract: dict[str, Any]) -> dict[str, Any]:
    if interface_name == DEFAULT_INTERFACE_NAME:
        return dict(data)
    ...
    for key in ("source_to_staging_summary", "staging_to_target_summary", "uniqueness_summary"):
        value = data.get(key)
        if isinstance(value, dict) and isinstance(value.get("items"), list):
            cloned[key] = {
                **value,
                "items": filter_items_by_interface(value.get("items"), interface_name, contract),
            }
...
def build_interface_page_sections(
    ...
    return [
        ...
        ("源到中间层核验结果", render_key_value(page_data.get("source_to_staging_summary"))),
        ("中间层到目标核验结果", render_key_value(page_data.get("staging_to_target_summary"))),
        ("总体判定依据", render_key_value(page_data.get("full_chain_consistency_summary"))),
        ("差异样例", render_difference_samples(entry["sample_items"], contract)),
        ("接口结论", render_conclusion_section(page_data, page_counts)),
    ]


def write_json_output(output_json: Path, data: dict[str, Any], mode: str, entries: list[dict[str, Any]], counts: dict[str, int]) -> None:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="渲染两阶段迁移报告 HTML。")
...
def main() -> int:
    ...
    write_json_output(Path(args.output_json), data, mode, entries, counts)
    ...
    subtitle = f"交付模式: {mode} | 生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    output_html.parent.mkdir(parents=True, exist_ok=True)

    if not mode_policy["requires_main_html"]:
        entry = entries[0]
        page_data = interface_view(data, entry["name"], contract)
        page_counts = count_fields(page_data)
        page_sections = build_interface_page_sections(page_data, entry, contract)
        page_html = fill_template(
            template,
            title=f"{title} - {entry['display_name']}",
            subtitle=f"单接口明细报告 | 生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
    ...
    if mode_policy["requires_interface_pages"] and interface_dir is not None:
        ...
        for entry in entries:
            ...
            page_html = fill_template(
                template,
                title=f"{title} - {entry['display_name']}",
                subtitle=f"分接口明细页 | 生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                cards=summary_cards(page_data, [entry], page_counts),
                sections=page_sections,
            )
