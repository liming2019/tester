...
1. 标准 UI 自动化测试用例文件路径，例如 `testcases/*UI自动化测试用例.md`，用例明细必须包含非空 `测试标题` 列；历史文件使用 `用例标题` 时可兼容读取，但新资产统一使用 `测试标题`。
...
2. 读取每条用例的 `用例ID` 和 `测试标题`，后续测试函数、注释、执行结果和报告映射必须保留这两个字段。
3. 若输入用例缺少 `测试标题` 或标题为空，先停止生成脚本并要求回到 `generate-ui-automation-testcases` 补齐，不要用脚本函数名替代测试标题。
4. 识别页面是否已有可复用的 Page Object 或 state setup；已有时优先增量更新。
5. 将同一页面脚本按以下固定结构拆分：
   - `test_<page>_smoke.py`
   - `test_<page>_readonly.py`
   - `test_<page>_write.py`
   - `test_<page>_role.py`（仅在存在角色差异或后续明确需要时生成）
6. 将只读场景统一并入 `readonly`，包括展示、筛选、分页、默认选中、弹窗打开、表单必填提示和只读字段校验。
7. 将所有会修改业务数据的场景统一并入 `write`，包括新增、编辑、删除、切换状态、取消关联和批量操作。
8. 为 `write` 脚本默认加显式写保护开关；只有用户明确允许，才真正执行写入。
9. 若脚本内会真实写入数据，必须一并生成清理或回滚策略，但这类策略仍属于代码资产，不属于执行报告。
...
- 生成的测试代码中必须能追溯 `用例ID` 和 `测试标题`，避免后续执行报告只展示脚本函数名。
